...
function createPouchMiddleware(_paths) {
  ...
  function listen(path, dispatch) {
    ...
    path.db.allDocs({
      ...
      dispatch({ type: 'init_data_load', payload: info });

      const changes = path.db.changes({ include_docs: true, batch_size: 500 });  // ({ live: true, include_docs: true });
      init = true;
      changes
        .on('change', change => {
          dispatch({ type: 'init_data_loading_step' });
          onDbChange(path, change, dispatch, loggedIn);
        })
        ...
        .on('complete', (res) => {
          ...
          path.db.changes({ live: true, include_docs: true, batch_size: 500, since: 'now' })
          .on('change', change => {
            onDbChange(path, change, dispatch, loggedIn);
          }).on('paused', (err) => {
            console.log('paused');
